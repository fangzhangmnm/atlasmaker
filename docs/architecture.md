...
**工具切换**
- `S` 选择工具（默认）
- `H` / `Space` 平移工具（Space 是临时态，按住时切，松开回上一个）
- `R` viewport 工具（拖框出画面）
- `I` Eyedropper（一次性，单击 image 后自动回 S）

**对象操作**
- `Ctrl+V` Smart paste：恰好选中 1 张未锁 image = 替换其 blob（保留几何）；否则 = 新建
- `Ctrl+C` 复制选中 image 到系统剪贴板
- `Ctrl+D` 复制选中对象（新 obj、独立 blob 字节、新 uuid src，不共享任何状态）
...
**保存**
- `Ctrl+S` 完全保存：本地 IDB + 云推（如登录中）。412 冲突 → toast 提示改名重试
- `Ctrl+Shift+S` 本地 only：明确不触云（用户在场版的「autosave 同力度」）
...
**全局 keydown + input focus = 静默删对象**：window-level `keydown` 监听对裸键（X / Backspace / Delete / H / R / S / I / 0 / Esc）都做映射。如果用户 focus 在 `<input>`（顶栏 sessionInput、gallery currentName、password dialog 等），不加 guard 会把这些键当 scene 快捷键跑。后果：

- 用户编辑 session 名按 Backspace 删字符 → 实际删了选中的 obj
- 输密码瞬间按 X / I / S → 删 obj / 切工具 / 不可见的 chaos
- 这正是 0.9.0 之前疑似「丢图」bug 的一类可能成因（另一类是 [[persistence-and-encryption]] 里 `_idSeq` 撞 id）

**修复**：[input.js](../src/input.js) `_onKeyDown` 顶部 guard
```js
const inInput = tgt && (tgt.tagName === "INPUT" || tgt.tagName === "TEXTAREA" || tgt.isContentEditable);
```
凡是焦点在文本输入元素 → 不跑 scene 快捷键。但 **Ctrl+S / Ctrl+Shift+S 例外处理 in guard 之前**：

```js
if ((ev.ctrlKey || ev.metaKey) && (ev.key === "s" || ev.key === "S")) {
  ev.preventDefault(); // 永远阻止浏览器「保存网页」
  if (ev.shiftKey) this.hooks.onSaveLocal?.();
  else this.hooks.onSave?.();
  return;
}
if (inInput) return;
```
...
理由：
- Ctrl+S 在 sessionInput / galleryCurrentName 改名时应该 work（保存名字改动是合理意图）
- preventDefault 永远跑，浏览器「保存网页」不弹
- 密码 dialog 期间按 Ctrl+S：hook 调到 saveCurrentSession，它检查 `_loading` → refuse + toast「Still loading — try again in a moment」。半完成态不污染 IDB。

`_loading` 在 `loadCurrentSession` / `applyAtlasZipBlob` / cloud pull-and-open 包裹时 = true，覆盖整个密码 prompt + 解密窗口期。

**Pointer 事件（crop 模式）**：进 crop mode 后给 body 加 `data-crop-mode="1"`，[input.js](../src/input.js) `_onPointerDown` 检查：除 pan（中键 / 空格）外一律 swallow。否则误点别的 obj 把 selection 换走，crop state 就指向不再选中的 obj。
...
- ~~**Duplicate 共享 object URL**：Ctrl+D 出来的拷贝和原图 src 指向同一 blob URL~~（**0.9.7 fixed**）：duplicate 强制新 uuid src + `new Blob([oldBlob], {type})` 同步深拷贝字节，完全独立。Vision：「同一张图出现两次 = 存两份」，不偷懒共享。
...
- **Crop 不支持 rotation**：obj.rotation ≠ 0 进 crop mode 会被 toast 拦下（要求先重置旋转）。math 上可以做（旋转 crop rect 的 offset 向量），但 UX 设计成 axis-aligned 更直觉；将来真要做就用 SVG 画旋转矩形。
